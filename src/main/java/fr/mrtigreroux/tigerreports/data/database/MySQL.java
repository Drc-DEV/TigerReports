...
public class MySQL extends Database {
    ...
    public void initialize() {
        Bukkit.getScheduler().runTaskAsynchronously(TigerReports.getInstance(), () -> {
            update("CREATE TABLE IF NOT EXISTS tigerreports_users (uuid char(36) NOT NULL, name varchar(20), cooldown varchar(20), immunity varchar(20), notifications text, true_appreciations int(5) DEFAULT 0, uncertain_appreciations int(5) DEFAULT 0, false_appreciations int(5) DEFAULT 0, reports int(5) DEFAULT 0, reported_times int(5) DEFAULT 0, processed_reports int(5) DEFAULT 0, PRIMARY KEY (uuid)) ENGINE=InnoDB CHARACTER SET=utf8mb4",
                    null);
            update("CREATE TABLE IF NOT EXISTS tigerreports_reports (report_id int(11) NOT NULL AUTO_INCREMENT, status varchar(50) NOT NULL DEFAULT 'Waiting', appreciation varchar(255), date varchar(20) NOT NULL, reported_uuid char(36) NOT NULL, reporter_uuid varchar(255) NOT NULL, reason varchar(255), reported_ip varchar(46), reported_location varchar(255), reported_messages text, reported_gamemode char(10), reported_on_ground char(1), reported_sneak char(1), reported_sprint char(1), reported_health varchar(10), reported_food varchar(10), reported_effects text, reporter_ip varchar(46) NOT NULL, reporter_location varchar(255) NOT NULL, reporter_messages text, archived tinyint(1) NOT NULL DEFAULT 0, PRIMARY KEY (report_id)) ENGINE=InnoDB CHARACTER SET=utf8mb4",
                    null);
